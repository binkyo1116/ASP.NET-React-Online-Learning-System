...
namespace RESTModelFunctions.Controllers
{
    ...
    {
        //public static void CalculateGrade(Activity activity, Student student, out int quizGrade, out int totalGrade, out int totalShown)
        ...
        {
            //int maxQuizGrade = activity.QuizQuestions.Select(qq => qq.MaxGrade).Sum();
            int maxQuizGrade = GetMaxQuizGrade(activity.Id);
            //System.Collections.Generic.IEnumerable<StudentQuizQuestion> qqStudentAnswers = activity.QuizQuestions.Select(qq => qq.StudentQuizQuestions.Where(sqq => sqq.StudentId == student.StudentId).OrderByDescending(q => q.Id).FirstOrDefault()).Where(qa => qa != null);
            ...
            //int answerShownCount = qqStudentAnswers.Where(qqa => qqa.AnswerShown).Select(qqa => qqa.QuizQuestion.MaxGrade).Sum();
        ...
        public static void CalculateQuizGrade(List<QuestionController.ResultInfo> quizData, out int quizGrade, out int totalGrade, out int totalShown)
        {
